...
const ProductDetail = () => {
  ...
  useEffect(() => {
    // لما id تتغير (يعني المستخدم ضغط على منتج تاني) الصفحة بتتحدث تلقائي
    const productsArray = productsData.products || [];
    const productId = parseInt(id);
    const foundProduct = productsArray.find((p) => p.id === productId);
    setProduct(foundProduct);
    ...
    if (foundProduct) {
      // المنتجات المشابهة حسب الاسم أو البراند
    ...
    setLoading(false);
  }, [id]); // 👈 مهم جدًا — يخلي الصفحة تتحدث لما المستخدم يضغط على منتج مشابه
          ...
          <div className="related-list">
            {relatedProducts.map((item) => (
              <div
                ...
                onClick={() => navigate(`/products/${item.id}`)} // 👈 هنا السحر
              >
